    ...
    class AppEngine
    {
        ...
        public Student GetStudentById(int sid)
        {
            ...
            if (dr != null)
            {
                while (dr.Read())
                {
                    student = new Student((int)dr["studentId"], (string)dr["studentName"], (string)dr["dob"]);
                }
            }
            con.Close();
            return student;
        ...
        public Course GetCourseById(int cid)
        {
            ...
            SqlDataReader dr;
            con = SQLConnection.getcon();
            cmd = new SqlCommand("select * from Course where CourseId=@cid", con);
            ...
            if (dr != null)
            {
                while (dr.Read())
                {
                    course = new Course((int)dr["CourseId"], (string)dr["CourseName"], (int)dr["CourseDuration"], Convert.ToDouble(dr["CourseFee"]));
                }
            }
            con.Close();
            return course;
